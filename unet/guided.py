...
def make_mask(labels, p):
    mask = torch.rand(labels.shape, device=labels.device) > p
    return rearrange(mask, 'b -> b 1')
...
class GuidedUnet(nn.Module):
    def __init__(
        ...
        self.cemb_dim = self.temb_dim

        self.input_proj = nn.Conv2d(3, self.ch, 3, 1, 1)
        
        self.null_class_emb = nn.Parameter(torch.randn(self.ch))
        self.class_emb = nn.Embedding(num_classes, self.ch)
    ...
    def forward(self, x, t, classes, uncond = False):
        ...
        assert classes.shape == (x.shape[0],), 'a classes for each x in the batch'

        batch_size = x.shape[0]

        if self.training:
            cemb = self.class_emb(classes)
            null_cemb = repeat(self.null_class_emb, 'd -> b d', b = batch_size)

            mask = make_mask(classes, self.p_uncond)
            ...
            cemb = self.class_proj(cemb)
        else: 
            if uncond: # Allows unconditional generation during inference
                cemb = repeat(self.null_class_emb, 'd -> b d', b = batch_size)
            else:
                cemb = self.class_emb(classes)
        ...
        emb = temb + cemb # TODO check other methods like cat
